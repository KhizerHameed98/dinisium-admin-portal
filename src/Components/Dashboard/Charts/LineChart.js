...
function MarketcapLineChart({ chartData }) {
  const [text, setText] = useState("Month");
  const [maxNum, setMaxNum] = useState(0);
  const data = chartData;
  console.log(data);
  console.log("Line Charts=================>", data);
  const dispatch = useDispatch();

  for (var i = 0; i < data?.length; i++) {
    if (parseInt(data[i].value) > maxNum) {
      setMaxNum(parseInt(data[i].value));
    }
  }
      ...
      >
        <CSSTransition in={true} appear={true} timeout={600} classNames="fade">
          ...
          >
            ...
            <Line dataKey={"value"} stroke="#8884d8" activeDot={{ r: 6 }} />
          </LineChart>
        </CSSTransition>
      </ResponsiveContainer>
    </div>
...
export default MarketcapLineChart;
